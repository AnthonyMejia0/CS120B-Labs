...
void print_substr(char s[], unsigned char start, unsigned char finish) {
	unsigned char size = finish - start;
	unsigned char pos = 0;
	char tempString[size];
	
	for (unsigned char j = start; j < size; j++) {
		tempString[pos] = s[j];
		pos++;
	}
	...
	LCD_ClearScreen();
	LCD_DisplayString(1, newString);
}

//--------End Print Substring Function------------------------------------------
...
enum LCDStates {init, firstPass, secondPass};
	
int LCDTick(int state) {
	static unsigned char cursor;
	static unsigned char stringPos;
	char string[] = "CS120B is Legend... wait for it DARY!";
	size_t arraySize = sizeof(string)/sizeof(char);
	...
	switch (state) {
		...
		case firstPass:
			if (cursor > 0) {
				LCD_ClearScreen();
				LCD_DisplayString(cursor, string);
				cursor--;
			}
			else if (cursor == 0){
				state = secondPass;
		...
		case secondPass:
			if (stringPos < arraySize) {
				print_substr(string, stringPos, arraySize);
				stringPos++;
			}
			break;
	}
